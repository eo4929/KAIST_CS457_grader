students = []
input_filename = './attend/participants_cs459_lec8_20210930.csv'
outout_filename = 'out_attendance_0930.csv'

with open('students_cs459.csv', 'r', encoding='utf-8') as f: # 학생 리스트
    f.readline()
...
not_late = duration * 0.8 - 5
absent = duration * 0.5 - 5
...
for i in range(0, len(lines)): # 학생당 출석/지각/결석 여부 -> attendance 변수에 저장
    if lines[i] != '\n':
        items = lines[i].rstrip('\n').split(',')
